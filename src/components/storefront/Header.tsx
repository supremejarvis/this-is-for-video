...
import { 
  ...
  Building2, User, LayoutDashboard, Truck, Bell, Flame, CheckCircle2, Phone, Mail, Sparkles, Wrench, Info, Heart, Globe 
} from 'lucide-react';
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5 flex-shrink-0">
          ...
          <div className="relative">
            <button
              ...
              className="h-10 flex items-center gap-1.5 px-2.5 sm:px-3 rounded-xl bg-white hover:bg-slate-100 border border-slate-200 text-slate-800 font-bold text-xs shadow-sm transition-all focus-visible:ring-2 focus-visible:ring-[#0054A6] shrink-0 cursor-pointer"
            >
              <Globe className="w-4 h-4 text-[#0054A6] shrink-0" />
              <span className="hidden sm:inline text-xs font-semibold">
                {SUPPORTED_LANGUAGES.find(l => l.code === (selectedLanguage || 'en'))?.label || 'English'}
              </span>
              <span className="sm:hidden text-xs uppercase font-bold">
                {(selectedLanguage || 'en').toUpperCase()}
              </span>
              <ChevronDown className="w-3 h-3 text-slate-400" />
            </button>

            {isLangOpen && (
              <div className="absolute right-0 top-full mt-1 w-36 bg-white border border-slate-200 rounded-xl shadow-xl z-50 py-1 text-xs overflow-hidden">
                {SUPPORTED_LANGUAGES.map((lang) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setIsLangOpen(false);
                      useStore.getState().showToast(`Language set to ${lang.label} (${lang.nativeName})`, 'info');
                    }}
                    className={`w-full text-left px-3 py-2 flex items-center justify-between hover:bg-blue-50 transition-colors cursor-pointer ${
                      (selectedLanguage || 'en') === lang.code ? 'bg-blue-50 text-[#0054A6] font-bold' : 'text-slate-700'
                    }`}
                  >
