...
export const SuperAdminDashboard: React.FC = () => {
  const { 
    ...
    fetchApiCatalog, apiCatalogLoading, apiCatalogError, apiCatalogProducts
  } = useStore();
      ...
      {activeAdminTab === 'PRODUCTS' && (
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              {/* PostgreSQL Live Auto-Sync Indicator & Button */}
              <button
                onClick={() => fetchApiCatalog()}
                disabled={apiCatalogLoading}
                className={`px-3.5 py-2 font-bold text-xs rounded-xl border shadow-2xs flex items-center gap-1.5 transition-all hover:scale-[1.02] ${
                  apiCatalogLoading
                    ? 'bg-slate-100 text-slate-500 border-slate-300'
                    : 'bg-emerald-50 hover:bg-emerald-100 text-emerald-950 border-emerald-300'
                }`}
                title="Synchronize live product catalog with PostgreSQL database"
              >
                <RefreshCw className={`w-3.5 h-3.5 text-emerald-600 ${apiCatalogLoading ? 'animate-spin' : ''}`} />
                <span>{apiCatalogLoading ? 'Syncing...' : 'Auto-Sync with PostgreSQL'}</span>
              </button>

              {/* Direct link to Buyer Catalog Storefront */}
              <a
                href="/store"
                target="_blank"
                rel="noopener noreferrer"
                className="px-3.5 py-2 bg-blue-50 hover:bg-blue-100 text-[#0054A6] font-bold text-xs rounded-xl border border-blue-300 shadow-2xs flex items-center gap-1.5 transition-transform hover:scale-[1.02]"
                title="Open Buyer Catalog Storefront in new tab"
              >
                            ...
                            <div className="space-y-1.5 max-w-[420px]">
                              ...
                              <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
                                ...
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-blue-50 hover:bg-blue-100 text-[#0054A6] text-[10px] font-bold border border-blue-200 transition-colors shadow-2xs"
                                  title="Open live product detail page in Buyer Catalog"
                                >
